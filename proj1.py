...
def packaging_clauses(runnerList):
    ...
    # (6) Check any runner doesn't have a task to do while travelling

    for i in range(totalRunners):
        for t in range(maxTime):
            for j in range(totalProducts):
                for jprime in range(totalProducts):
                    if jprime !=j:
                        for tprime in range(maxTime):
                            tjjprime=timeList[j][jprime]
                            for jprimeprime in range(totalProducts):
                                if tprime>=t and tprime <(t+tjjprime):
                                    clausesList.append([-p(i, t+tjjprime, jprime), -p(i, tprime, jprimeprime)])
                                    clausesList.append([-p(i,t,j), -p(i, tprime, jprimeprime)])
...
def solve():
    ...
    formalize(runnersMov, conveyorMov)


def formalize(runnersMov, conveyorMov):
    if runnersMov == [] or conveyorMov == []:
        print ("UNSAT")
    else: 
        optiTime = 0
        package = 0
        for i in range(len(runnersMov)):
                if runnersMov[i][1]>optiTime:
                    optiTime = runnersMov[i][1]
                    package = runnersMov[i][2]
        optiTime+=int(movTimeConv[package-1])
        print(optiTime)
        for a in range(totalRunners):
            counter = 0
            tempList =[]
            for i in range(len(runnersMov)):
                if runnersMov[i][0]==a+1:
                    counter +=1
                    tempList.append(runnersMov[i][2])
            print (counter, end=" ")
            print (*tempList, sep = " ")
        
        for i in range(len(orderList)):
            print(len(orderList[i]), end =" ")
            for j in range(len(orderList[i])):
                for k in range(len(conveyorMov)):
                    if orderList[i][j] == conveyorMov[k][0]:
                        print(*conveyorMov[k], sep = ":", end= " ")
            print("")
